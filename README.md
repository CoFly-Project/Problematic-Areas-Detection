...
Τhe module takes as input the extracted VI arrays (npy file), the georeferenced VI image representations (tiff), and the path of *.txt file that contains the names of the UAV images (utilized for stitching). The output of this module is a json file containing for each detected point:
