...
Τhe module takes as input the extracted _VI arrays_ (npy files), the _VI image representations_ (tif files), and a _*.txt file_ that contains the names of the images. The names of the images should describe theirs geolocation, e.g. _```IMG_20_Lat=XXX,Lon=XXX.jpeg```_.<!--(utilized for stitching).-->The output of this module is a json file containing for each detected point:
